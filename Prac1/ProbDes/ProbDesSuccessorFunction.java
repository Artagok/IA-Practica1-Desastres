...
public class ProbDesSuccesorFunction implements SuccessorFunction {
    public List getSuccessors(Object state){
        ...
        ArrayList retval = new ArrayList();
            ProbDesBoard PGBoard = (ProbDesBoard) state;
            ProbdDesBoard PGBoardaux; // PGBoardAux ha de ser una copia de PGBoard --> cal picar una constructora "per copia"
            
            int numHelicopters = PGBoard.getNHelicopters();
            
            for (int heli = 0; heli < numHelicopters; heli++){
                
                ArrayList<int> grups = PGBoard.getGrupsHeli(heli); //<----- id dels grups
                for (int g = 0; g < grups.size(); g++){
                    ...
                    for (int sortida = 0; sortida < num_sortides; sortida++){
                        ...
                        if (PGBoard.noEsViolenRestriccions1(heli,g,sortida)  && sortida_origen != sortida){
                            ...
                            PGBoardaux = new ProbDesBoard(PGBoard);
            ...
            for (int heli1 = 0; heli1 < numHelicopters; heli1++){
                ...
                for (int heli2 = 0; heli2 < numHelicopters; heli2++){
                    ...
                    if (heli1 != heli2){
                        ArrayList<int> grups = PGBoard.getGrupsHeli(heli1); //<----- id dels grups
                        for (int g = 0; g < grups.size(); g++){
                            ...
                            for (int sortida = 0; sortida < num_sortides; sortida++){
                                ...
                                if (PGBoard.noEsViolenRestriccions1(heli2,g,sortida){
                                    ...
                                    retval.add (new Successor ("Moure Grup Diferen Helicopter", PGBoardaux); //falta el cas d'afegir una nova sortida amb un sol grup
                                }
                            }
                        }
                    }
            ...
            for (int heli1 = 0; heli1 < numHelicopters; heli1++){
                ...
                for (int heli2 = 0; heli2 < numHelicopters; heli2++){
                    ...
                    ArrayList<int> grups1 = PGBoard.getGrupsHeli(heli1);
                    for (int g1 = 0; g1 < grups1.size(); g1++){ 
                    int sortidaG1 = PGBoard.getNumSortida(heli1,grups.get(g1));
                    
                        ArrayList<int> grups2 = PGBoard.getGrupsHeli(heli2);
            ...
            for (int heli1 = 0; heli1 < numHelicopters; heli1++){
                ...
                for (int heli2 = 0; heli2 < numHelicopters; heli2++){ 
                    ...
                    ArrayList<int> viatgesH1 = PGBoard.getViatgesHeli(heli1);
            ...
            for (int heli1 = 0; heli1 < numHelicopters; heli1++){
                ...
                for (int heli2 = 0; heli2 < numHelicopters; heli2++){ 
                    ...
                    for (int v1 = 0; v1 < viatgesH1.size(); v1++){
                        ...
                        ArrayList<int> viatgesH2 = PGBoard.getViatgesHeli(heli2);
